...
// The number of non-isomorphic full binary trees with  internal nodes (i.e. nodes having at least one son).
// The number of monotonic lattice paths from point (0,0) to point (n,n) in a square lattice of size nxn, which do not pass above the main diagonal (i.e. connecting (0,0) to (n,n)).
// Number of permutations of length  that can be stack sorted (i.e. it can be shown that the rearrangement is stack sorted if and only if there is no such index i < j < k, such that ak < ai < aj).
// The number of non-crossing partitions of a set of  elements.
// The number of ways to cover the ladder 1..n using n rectangles (The ladder consists of n columns, where i column has a height i).
